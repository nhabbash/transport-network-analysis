...
![Partitioned metro network with $\gamma = 2km$, $\sigma = 5km$, stops colored according to the $C_i$ region they belong to](images/metro-centroids.png)
...
Since boundary regions can be neighbor to other distant regions through to-infinity borders (i.e. Voronoi Diagrams aren't bounded per se), a parameter $\sigma$ is given as a neighboring distance limit. The value chosen for this network is $\sigma = 5km$.
...
![Partitions of the public transport network with $\gamma = 2km$, $\sigma = 5km$, corresponds to the Delaunay triangulation of the Voronoi diagram](images/net-centroids.png)

![Cell-to-cell flow graph of the public transport network](images/net-c2c.png)

## Network Analytics

Analyzing the difference between the neighbor graph and the cell-to-cell graph can give an interesting insight into how well-connected are different city regions and how resilient each connection is. Centrality measures can help measuring these differences.

Nodes in the cell-to-cell graph with lower degree and eigenvector centralities represent regions weakly connected to the network - hence regions that might cause more troubles to the passengers of those regions if the stops were to undergo failures.

The nodes' closeness represent how much a node can influence other nodes, or spread information, in the network. Highly connected networks tend to have similar closeness for every node. This represents for transit systems a measure of how much a node can offer access to other nodes and spread passengers quickly to any destination of the network.

The nodes' betweenness represent how much a node has influence over flows in a graph. For transit systems this is a measure of "gateway" regions that act as a bridge between different regions.
